...
socketServer.listen(process.env.PORT || 8080, () => {
 ...
 */
  ...
  socket
    ...
    .on('request', (data) => {
      console.log('HELLO')
      console.log(socket.id, socket.handshake.session.username);
...
const invokeChatGuest = (sessionItemID, currentUserL) => {
  let currentSession = ChatSession.findById(sessionItemID);
    ...
    await socket.on('send uniqueRoomID', async (data) => {
    ...
    let privateRoute = await customRoom + 'Pmessage';
    console.log(privateRoute, '87656787658765678765876567876587656787658765678765876567876587656787658765678765876567876587656787658765678765876567876587656787658765678765876567876587656787658765678765876567876587656787658765678765876567876587656787658765678765')

    let privateRouteMessages = customRoom + 'Pmessages';
    socket.on(privateRoute, async (message) => {

    console.log(privateRouteMessages, '<---------1')
    if (socket.handshake.session.logged) {
      const msgObj = await {
        username: socket.handshake.session.username,
        message: message
        ...
        await messages.push(msgObj);
      console.log(messages);
        console.log(privateRouteMessages, '<---------2')
        await socketServer.in(customRoom).emit('privateRouteMessages', messages);
        console.log(privateRouteMessages, '<---------2/1/2')

      console.log(msgObj, 'msgObj');
    } else {
        await socketServer.in(customRoom).emit('privateRouteMessages', 'Incorrect Username Or Password');
    }
    });
    }, {
      scheduled: true,
...
let dTask = cron.schedule(newSesh.cronDestroyTime, async  () => {
  console.log('Destroy');
  await socket.emit('initiateRoomDestroy', 'DESTROY');
  await task.destroy();
  await dTask.destroy();
  await socket.leave(customRoom);
  ...
  const invokeChat = (sessionItemID, currentUserL) => {
    ...
    let currentSession = ChatSession.findById(sessionItemID);
      ...
      await socket.on(privateRoute, async (message) => {
      if (socket.handshake.session.logged) {
        const msgObj = {
          username: socket.handshake.session.username,
          message: message
  ...
  let dTask = cron.schedule(newSesh.cronDestroyTime, async () => {
    ...
    let sessionIndex = await currentUserL.ownChats.indexOf(newSesh._id);
    await currentUserL.ownChats.splice(sessionIndex, 1);
    await currentUserL.save();
